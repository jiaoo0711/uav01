...
def get_args(description='DGL on Retrieval Task'):
    ...
    parser.add_argument('--temporal_prompt', default="", type=str,
                        help='temporal prompt for learnable prompt in efficient prompt tuning')
    parser.add_argument('--hard_mask_rate', default='0.7', type=float, help='mask rate for hard negatives')
    parser.add_argument('--num_props', default=1, type=int, help='number of proposal boxes for each video')
    parser.add_argument('--num_prompts', default=16, type=int, help='number of prompts for efficient prompt')
    parser.add_argument('--top_frames', default=8, type=int, help='number of selected frames for prompts learning')
    
    # FG-CLIP相关参数
    parser.add_argument('--use_fgclip', action='store_true', 
                        help='Use FG-CLIP instead of original CLIP')
    parser.add_argument('--fgclip_model_name', default='qihoo360/fg-clip-base', type=str,
                        help='FG-CLIP model name or path')
    parser.add_argument("--fgclip_local_path", type=str, default=None,
                            help="Local path to the pretrained FG-CLIP model and tokenizer.")
    parser.add_argument("--fg_clip_max_len", type=int, default=248,
                            help="Maximum sequence length for FG-CLIP tokenizer.")
    parser.add_argument("--num_unfrozen_layers", type=int, default=0, 
                            help="Number of last FG-CLIP layers to unfreeze for fine-tuning.")
