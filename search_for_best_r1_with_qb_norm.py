...
def compute_metrics(x):
    ...
    # 扩展到R1-R10的完整范围
    for k in range(1, 11):
        metrics[f'R{k}'] = float(np.sum(ind < k)) * 100 / len(ind)
...
def search_best_beta(train_test_t2v, train_test_v2t, sim_matrix, beta_range=(0.5, 1.0), beta_step=0.01):
    ...
    for beta in np.arange(beta_range[0], beta_range[1] + beta_step, beta_step):
        ...
        # 显示R1-R10的完整范围
        recall_str = " - ".join([f"R@{k}: {tv_metrics[f'R{k}']:.1f}" for k in range(1, 11)])
        info_str.append(f' (metric) >>> {recall_str} - Median R: {tv_metrics["MR"]:.1f} - Mean R: {tv_metrics["MeanR"]:.1f}')
        
        info_str.append("Video-to-Text:")
        # 显示R1-R10的完整范围
        recall_str_vt = " - ".join([f"V2T$R@{k}: {vt_metrics[f'R{k}']:.1f}" for k in range(1, 11)])
        info_str.append(f' (metric) >>> {recall_str_vt} - V2T$Median R: {vt_metrics["MR"]:.1f} - V2T$Mean R: {vt_metrics["MeanR"]:.1f}')
